 ...
 *
 * @implNote При получении {@code MatrixUserInfo} сервис {@code UserService} проверяет
 *           существование пользователя по внешнему Matrix ID и при необходимости создаёт
 *           новую запись с базовым профилем.
...
public interface UserProvisioningPort {
     ...
     * @param userInfo   информация, полученная от Matrix Federation
     * @return идентификатор локального пользователя
     */
    Long findByIdOrCreate(String externalId, MatrixUserInfo userInfo);
